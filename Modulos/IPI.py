...
def Le_IPI2(n,m,paciente,samplefrom):
    print("PACIENTE: ",paciente)
    IPIs = []
    start_at = 0
    if(paciente<10):
        ...
        paciente = str(paciente)
    for i in range(n,m):
        record = wfdb.rdrecord('samplesnovos/Person_'+paciente+'/rec_'+str(i+1), physical=False, sampfrom= samplefrom, channel_names= ['ECG I filtered'])
        print(len(record.d_signal))
        print(record.sig_name)
        print(record.fs)  #frequencia
        sig = []
